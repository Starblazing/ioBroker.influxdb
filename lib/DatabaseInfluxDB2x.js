...
class DatabaseInfluxDB2x extends Database {
    ...
    async deleteData(start, stop, org, bucket, predicate, callback_error) {
       try {
    ...
    stateValueToPoint(pointName, stateValue) {
        ...
        if (this.useTags) {
            point = new InfluxClient.Point(pointName)
                ...
                if (stateValue.customTags) {
                    stateValue.customTags.map(customTag => {
                        point.tag(customTag.name, customTag.value);
                    });
                }
        } else {
            point = new InfluxClient.Point(pointName)
